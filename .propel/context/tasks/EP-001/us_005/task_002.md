...
us_id: us_005
epic: EP-001
title: "Implement Profile CRUD Service with Completion Calculation"
status: done
layer: backend
effort: 4h
priority: critical
...
# TASK-002 — Implement Profile CRUD Service with Completion Calculation
...
**Addresses Acceptance Criteria**: Scenario 2 (profile update persists), Scenario 4 (full CRUD), Scenario 1 (completion tracking)

The profile service manages all CRUD operations for candidate profiles including basic info, skills, education, and work history. It must calculate profile completion percentage based on required fields and update the onboarding checklist state automatically.
...
- Creates new profiles with default completion state (0%)
- Updates profile sections (basic info, skills, education, work history)
- Calculates profile completion percentage dynamically
- Marks onboarding sections as complete when all required fields are filled
- Provides validation for profile data integrity
- Logs all profile changes to audit_events
...
|----------|-------|--------|--------------|
| `createProfile` | candidateId, profileData | Profile | Creates profile with 0% completion |
| `getProfileByCandidate` | candidateId | Profile \| null | Read-only |
| `updateProfile` | candidateId, updates | Profile | Recalculates completion, logs audit |
| `deleteProfile` | candidateId | boolean | Soft delete or cascade |
| `calculateCompletion` | profileData | ProfileCompletionStatus | Read-only calculation |
| `getCompletionStatus` | candidateId | ProfileCompletionStatus | Aggregates profile + consent data |

**Completion Calculation Logic**:
```typescript
// Each section worth 20% (5 sections × 20% = 100%)
const SECTION_WEIGHT = 20;

const sections = {
  basic_info: hasFullName && experienceYears >= 0,
  skills: skills.length >= 3,
  education: education.length >= 1,
  work_history: workHistory.length >= 1,
  privacy_consent: privacyConsentExists,
};

percentage = Object.values(sections).filter(Boolean).length * SECTION_WEIGHT;
```
...
Create `backend/src/utils/profileValidator.ts`:
...
- [x] `profileService.ts` created with create, read, update, delete, and completion calculation functions
- [x] Profile completion calculated as percentage (0-100) based on 5 sections
- [x] Minimum requirements: 3 skills, 1 education entry, 1 work history entry
- [x] All profile changes logged to `audit_events` table
- [x] Validation errors returned with clear messages
- [x] Completion percentage updates automatically on profile save
- [x] Service handles partial updates correctly (skills-only, education-only, etc.)
...
- Prisma Client with extended schema
...
describe('profileService', () => {
  it('creates profile with 0% completion for empty data');
  it('calculates 20% completion when basic info complete');
  it('calculates 40% completion when basic info + skills complete');
  it('calculates 100% completion when all sections complete');
  it('updates profile and recalculates completion');
  it('validates minimum 3 skills requirement');
